...
// export default function AddHotspotPage({facilitiesId, roomId, sceneId}) {
...
//     previewImage.src = `/tiles/${facilitiesId}/${roomId}/${sceneId}/preview.jpg`;
//     previewImage.onload = () => {
...
//   // Handler to send hotspot data via API.
//   const handleSaveHotspots = async () => {
//     console.log("Custom Hotspots Data:", customHotspots);
//     try {
//       const response = await fetch('/api/saveHotspots', {
//         method: 'POST',
//         headers: {
...
//       console.log("Hotspots saved successfully:", data);
//       alert("Hotspots saved successfully!");
//     } catch (error) {
//       console.error("Error saving hotspots:", error);
//       alert("Error saving hotspots. Please try again.");
//     }
//   };
...
// export default function AddHotspotPage({ facilitiesId, roomId, sceneId, hotspotName }) {
//   const containerRef = useRef(null);
//   const [customHotspots, setCustomHotspots] = useState([]);
//   // Ref to store the currently dragged hotspot (if any).
//   const draggingHotspotRef = useRef(null);
...
//     console.log("Custom Hotspots Data:", customHotspots);
//     try {
//       const response = await fetch('/api/method/simsaar.api.saveHotspots', {
//         method: 'POST',
//         headers: {
//           'Content-Type': 'application/json',
//         },
...
export default function AddHotspotPage({ facilitiesId, roomId, sceneId }) {
  const containerRef = useRef(null);
  const [customHotspots, setCustomHotspots] = useState([]);
  // Save message to display after the API call.
    ...
    renderer.domElement.addEventListener('pointermove', (e) => {
      if (draggingHotspotRef.current) {
        ...
        if (raycaster.ray.intersectPlane(floorPlane, intersectionPoint)) {
          ...
          rootGroup.worldToLocal(localPoint);
          draggingHotspotRef.current.position.copy(localPoint);
          draggingHotspotRef.current.userData.position = [
            localPoint.x,
            localPoint.y,
            localPoint.z,
          ];
    ...
    renderer.domElement.addEventListener('pointerup', (e) => {
      ...
      if (dist <= 5) {
        ...
        if (raycaster.ray.intersectPlane(floorPlane, intersectionPoint)) {
          ...
          // Ask the user for the target room for this hotspot.
          const targetName = window.prompt("Enter target room for hotspot:");
          if (targetName) {
            const newHotspot = {
              target: targetName,
              position: [localPoint.x, localPoint.y, localPoint.z],
  ...
  // Handler to send hotspot data via API.
  const handleSaveHotspots = async () => {
    // Display a prompt to enter the view hotspot name
    const hotspotName = window.prompt("Enter view hotspot name:");
    if (!hotspotName) {
      alert("View hotspot name is required.");
      return;
    }
    try {
      const response = await fetch('/api/method/simsaar.api.saveHotspots', {
      ...
      const data = await response.json();
      console.log('Hotspots saved successfully:', data);
      setSaveMessage('Hotspots saved successfully!');
    } catch (error) {
      console.error('Error saving hotspots:', error);
      setSaveMessage('Error saving hotspots. Please try again.');
    }
  };
  ...
  return (
    <>
      ...
      >
        <button onClick={handleSaveHotspots}>Save Hotspots</button>
        {saveMessage && (
          <div style={{ marginTop: '10px', fontWeight: 'bold' }}>{saveMessage}</div>
        )}
      </div>
    </>
  );
